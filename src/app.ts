(() => {
  ...
  type Move = { from: MoveFrom; to: number };
  type GamePhase = 'opening' | 'rolling' | 'moving' | 'gameover';
  type WinType = 'normal' | 'gammon' | 'backgammon';
  ...
  interface CombinedDest {
    to: number;
    steps: Move[];
  }

  interface DicePoolEntry {
    d: number[];
    w: number;
  }
  ...
  interface StylesCache {
    ...
    checkerWhiteHi: string;
    checkerWhiteLo: string;
    checkerBlackHi: string;
    checkerBlackLo: string;
    checkerWhiteStroke: string;
    checkerBlackStroke: string;
    checkerWhiteRing: string;
    checkerBlackRing: string;
    labelWhite: string;
    labelYellow: string;
    labelLight: string;
    miniWhite: string;
    miniBlack: string;
    miniWhiteStroke: string;
    miniBlackStroke: string;
    dieWhiteBg: string;
    dieBlackBg: string;
    dieWhiteDots: string;
    dieBlackDots: string;
    ...
    hlBearoff: string;
    hlCombined: string;
    hlCombinedShadow: string;
    hlQueued: string;
    hlQueuedShadow: string;
    hlQueuedDest: string;
    hlQueuedDestShadow: string;
    openingOverlay: string;
    openingVs: string;
    openingDieWShadow: string;
    openingDieBShadow: string;
    openingDieBBg: string;
    fontPtLabel: string;
    fontCheckerCount: string;
    fontBarCount: string;
    fontSmallLabel: string;
    fontBorneCount: string;
    fontOpeningVs: string;
    fontOpeningName: string;
    dieSize: number;
    dieCornerR: number;
    dieDotR: number;
    dieDotOffset: number;
    openingDieSize: number;
    openingDieCornerR: number;
    openingDieDotR: number;
    openingDieDotOffset: number;
    openingDieStrokeW: number;
    openingDieShadowBlur: number;
    winMsgColor: string;
    winGlowColor: string;
    lossMsgColor: string;
    lossGlowColor: string;
    winLineH: number;
    boardBgImgAlpha: number;
    boardOverlay: string;
    pointInlay: string;
    dieInlay: string;
    medallionOuter: string;
    medallionInner: string;
    checkerWhiteRosette: string;
    checkerBlackRosette: string;
  }

  const canvas = document.getElementById('board') as HTMLCanvasElement;
  const ctx = canvas.getContext('2d') as CanvasRenderingContext2D;
  const rollBtn = document.getElementById('rollBtn') as HTMLButtonElement;
  const rollOverlayBtn = document.getElementById('rollOverlayBtn') as HTMLButtonElement;
  ...
  const geminiBtn = document.getElementById('geminiBtn') as HTMLButtonElement;
  const apiKeyInput = document.getElementById('apiKeyInput') as HTMLInputElement;
  const apiKeyToggle = document.getElementById('apiKeyToggle') as HTMLButtonElement;
  const diffSel = document.getElementById('difficulty') as HTMLSelectElement;
  const statusEl = document.getElementById('status') as HTMLElement;
  const whiteScoreEl = document.getElementById('white-score') as HTMLElement;
  const blackScoreEl = document.getElementById('black-score') as HTMLElement;
  const humanPtsEl = document.getElementById('human-pts') as HTMLElement;
  const computerPtsEl = document.getElementById('computer-pts') as HTMLElement;
  const continueBtn = document.getElementById('continueBtn') as HTMLButtonElement;

  let W: number, H: number, MARGIN: number, PW: number, PH: number, BAR_W: number, BEAR_W: number, CR: number;
  let LEFT_X: number, BAR_X: number, RIGHT_X: number, BEAR_X: number, BOARD_TOP: number, BOARD_BOT: number;

  let board: number[] = new Array(25).fill(0);
  board[24] = 2; board[13] = 5; board[8] = 3; board[6] = 5;
  board[1] = -2; board[12] = -5; board[17] = -3; board[19] = -5;
  let barW = 0, barB = 0, offW = 0, offB = 0;
  let dice: number[] = [], movesLeft: number[] = [], currentPlayer: Player | null = null, gamePhase: GamePhase = 'gameover';
  let selectedSrc: MoveFrom | null = null, validDests: number[] = [];
  let isGeminiEnabled = true;
  let combinedDests: CombinedDest[] = [];
  let pickedQueue: MoveFrom[] = [];
  let difficulty: string = diffSel.value;
  let snapshot: BoardSnapshot | undefined;
  let openingRoll: { human: number | null; computer: number | null } = { human: null, computer: null };
  let humanColor: Player = 'white';
  let humanName = 'Kojany';
  let computerName = 'Pcyushka';
  let humanMatchScore = 0;
  let computerMatchScore = 0;
  let lastGameWinner: 'human' | 'computer' | null = null;
  ...
  const getComputerLabel = (): string => `${computerName} (${humanColor === 'white' ? 'Black' : 'White'})`;

  const showRollOverlay = (): void => { rollOverlayBtn.classList.add('visible'); };
  const hideRollOverlay = (): void => { rollOverlayBtn.classList.remove('visible'); };

  const requestPlayerNames = (): void => {
  ...
  const restoreSnapshot = (): void => {
    ...
    barW = snapshot.barW; barB = snapshot.barB;
    offW = snapshot.offW; offB = snapshot.offB;
    dice = [...snapshot.dice];
  ...
  const pointCenterX = (p: number): number => {
    if (p >= 13 && p <= 18) return LEFT_X + (p - 13) * PW + PW / 2;
    if (p >= 19 && p <= 24) return RIGHT_X + (p - 19) * PW + PW / 2;
    if (p >= 7 && p <= 12) return LEFT_X + (12 - p) * PW + PW / 2;
    return RIGHT_X + (6 - p) * PW + PW / 2;
  };

  const isTop = (p: number): boolean => p >= 13;

  const checkerPos = (p: number, idx: number): { x: number; y: number } => {
  ...
  const getAllLegalMoves = (mLeft: number[], player: Player): Move[] => {
    const moves: Move[] = [];

    const tryDie = (from: MoveFrom, die: number): void => {
      if (from === 'bar') {
        if (player === 'white') {
          const to = 25 - die;
          if (canLand(to, 'white')) moves.push({ from: 'bar', to });
        } else {
          const to = die;
          if (canLand(to, 'black')) moves.push({ from: 'bar', to });
        }
        return;
      }
      if (player === 'white' && isWhiteHome()) {
        if (die === from) {
          ...
          const to = from - die;
          if (to >= 1 && canLand(to, 'white')) moves.push({ from, to });
        }
        return;
      }
      if (player === 'black' && isBlackHome()) {
        ...
        if (die === (25 - from)) {
          ...
          const to = from + die;
          if (to <= 24 && canLand(to, 'black')) moves.push({ from, to });
        }
        return;
      }
      const to = player === 'white' ? from - die : from + die;
      if (to >= 1 && to <= 24 && canLand(to, player)) moves.push({ from, to });
    };
  ...
  const hasAnyValidMoves = (): boolean => getAllLegalMoves(movesLeft, currentPlayer!).length > 0;
  ...
  const applyMove = (from: MoveFrom, to: number): void => {
    ...
    if (from === 'bar') {
      if (player === 'white') barW--; else barB--;
    } else {
      if (player === 'white') board[from]--; else board[from]++;
    }

    if (to === 0) { offW++; return; }
    if (to === 25) { offB++; return; }

    if (player === 'white' && board[to] === -1) { board[to] = 0; barB++; }
    if (player === 'black' && board[to] === 1) { board[to] = 0; barW++; }
  ...
  const endGame = (winner: Player): void => {
    ...
    if (isHumanWin) humanMatchScore += points;
    else computerMatchScore += points;
  ...
  const doRoll = (): void => {
    if (gamePhase === 'opening') { doOpeningRoll(); return; }
    if (gamePhase !== 'rolling' || currentPlayer !== humanColor) return;
    hideRollOverlay();
  ...
  const startGame = (winner: 'human' | 'computer'): void => {
    openingRoll = { human: null, computer: null };
  ...
  const getWinType = (winnerColor: Player): WinType => {
    if (winnerColor === 'white') {
      if (offB > 0) return 'normal';
      const isBack = barB > 0 || [1,2,3,4,5,6].some(p => board[p] < 0);
      return isBack ? 'backgammon' : 'gammon';
    }
    if (offW > 0) return 'normal';
    const isBack = barW > 0 || [19,20,21,22,23,24].some(p => board[p] > 0);
    return isBack ? 'backgammon' : 'gammon';
  };

  const getWinMessage = (isHumanWin: boolean, winType: WinType): string => {
    if (isHumanWin) {
      if (winType === 'backgammon') return 'Congrats,\nYou won by Backgammon!!!';
      if (winType === 'gammon') return 'Congrats,\nYou won by Gammon!!';
      return 'Congrats,\nYou won!';
    }
    if (winType === 'backgammon') return 'Sadly you lost\nthis one by Backgammon';
    if (winType === 'gammon') return 'Sadly you lost\nthis one by Gammon';
    return 'Sadly you\nlost this one';
  };

  const startWinAnimation = (text: string, isHumanWin: boolean): void => {
    winMessage = { text, isHumanWin, startTime: performance.now() };
    const loop = (): void => {
      draw();
      if (winMessage) animFrameId = requestAnimationFrame(loop);
    };
    animFrameId = requestAnimationFrame(loop);
  };

  const stopWinAnimation = (): void => {
    if (animFrameId !== null) { cancelAnimationFrame(animFrameId); animFrameId = null; }
    winMessage = null;
  };

  const startDiceRollAnim = (finalDice: number[], cb: () => void): void => {
    const TOTAL_MS = 1000;
  ...
  const updateMatchScore = (): void => {
    humanPtsEl.textContent = `${humanName}: ${humanMatchScore} pts`;
    computerPtsEl.textContent = `${computerName}: ${computerMatchScore} pts`;
  };

  const getApiKey = (): string => apiKeyInput.value.trim();

  const pickGeminiSequence = async (cColor: Player): Promise<Move[]> => {
    const diff = diffSel.value;
    if (diff === 'easy') return pickEasySequence(cColor);

    const apiKey = getApiKey();
    if (!apiKey) return pickLocalSequence(cColor);

    const boardLines = Array.from({ length: 24 }, (_, i) => i + 1).flatMap(p =>
      board[p] > 0 ? [`Point ${p}: ${board[p]} white`] :
      board[p] < 0 ? [`Point ${p}: ${Math.abs(board[p])} black`] :
      []
    );
    if (barW > 0) boardLines.push(`White bar: ${barW}`);
    if (barB > 0) boardLines.push(`Black bar: ${barB}`);
    ...
    const prompt =
      `You are playing backgammon as the ${cColor} player. ` +
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`API ${response.status}: ${(errData as any).error?.message || response.statusText}`);
      }
      ...
      setStatus(`${computerName}: API error — ${(err as Error).message.slice(0, 60)}`);
      await new Promise(res => setTimeout(res, 800));
      return pickLocalSequence(cColor);
    }
  };

  const computerTurn = async (): Promise<void> => {
    const d1 = Math.floor(Math.random() * 6) + 1;
    const d2 = Math.floor(Math.random() * 6) + 1;
    const finalDice = [d1, d2];

    await new Promise<void>(res => startDiceRollAnim(finalDice, res));
  ...
  const validateAndTrimSequence = (seq: Move[], player: Player): Move[] => {
    ...
    const valid = seq.reduce<Move[]>((acc, move) => {
      if (valStopped) return acc;
      const legal = getAllLegalMoves(movesLeft, player);
      if (!legal.some(m => m.from === move.from && m.to === move.to)) { valStopped = true; return acc; }
      applyMove(move.from, move.to);
      acc.push(move);
      if (movesLeft.length === 0) valStopped = true;
      return acc;
    }, []);
  ...
  const getBestMoveSequence = (player: Player, evalFn: (c: Player) => number = evaluateHard): Move[] => {
    const bSnap = [...board], bwSnap = barW, bbSnap = barB;
    const owSnap = offW, obSnap = offB, mlSnap = [...movesLeft], cpSnap = currentPlayer;
    currentPlayer = player;
    ...
    const search = (seq: Move[]): void => {
      ...
      if (moves.length === 0) {
        ...
        if (s > bestScore) { bestScore = s; bestSeq = [...seq]; }
        return;
      }
      const seen = new Set<string>();
      moves.forEach(move => {
  ...
  const pickEasySequence = (player: Player): Move[] => {
    const bSnap = [...board], bwSnap = barW, bbSnap = barB;
    const owSnap = offW, obSnap = offB, mlSnap = [...movesLeft], cpSnap = currentPlayer;
    currentPlayer = player;
    const seq: Move[] = [];
  ...
  const evaluateEnhanced = (cColor: Player): number => {
  ...
  const DICE_POOL: DicePoolEntry[] = (() =>
    Array.from({ length: 6 }, (_, i) => i + 1).flatMap(a =>
      Array.from({ length: 6 - a + 1 }, (_, i) => i + a).map(b => ({
        d: a === b ? [a, a, a, a] : [a, b],
        w: a === b ? 1 : 2
      }))
    )
  ...
  const afterMove = (): void => {
    ...
    if (movesLeft.length === 0 || !hasAnyValidMoves()) {
      if (offW >= 15) { endGame('white'); return; }
      if (offB >= 15) { endGame('black'); return; }
      pickedQueue = []; selectedSrc = null; validDests = []; combinedDests = [];
    ...
    if (pickedQueue.length > 0) {
      ...
      if (validDests.length === 0) {
        selectedSrc = null; pickedQueue = []; combinedDests = [];
        ...
        const extras: string[] = [];
        if (combinedDests.length > 0) extras.push(`+${combinedDests.length} combined`);
        if (pickedQueue.length > 0) extras.push(`+${pickedQueue.length} still queued`);
        setStatus(`Queued move — go to: ${destStr}${extras.length ? ` (${extras.join(', ')})` : ''}`);
      }
    } else {
      selectedSrc = null; validDests = []; combinedDests = [];
      setStatus(`Move made. Remaining dice: [${movesLeft.join(', ')}]`);
    }
  };

  const findCombinedDestsFrom = (startSrc: MoveFrom): CombinedDest[] => {
    const player = currentPlayer!;
    const combined: CombinedDest[] = [];
    const saveState = () => ({
      board: [...board], barW, barB, offW, offB, movesLeft: [...movesLeft]
    });
    const restoreState = (s: ReturnType<typeof saveState>): void => {
      board = [...s.board]; barW = s.barW; barB = s.barB;
      offW = s.offW; offB = s.offB; movesLeft = [...s.movesLeft];
    };
    const dfs = (from: MoveFrom | null, steps: Move[]): void => {
      if (movesLeft.length === 0 || from === null) return;
  ...
  canvas.addEventListener('click', (e: MouseEvent) => {
    if (gamePhase !== 'moving' || currentPlayer !== humanColor) return;
    ...
    if (clicked === null) {
      selectedSrc = null; pickedQueue = []; validDests = []; combinedDests = [];
      draw(); return;
    }
    ...
    if (selectedSrc !== null && combinedDests.some(c => c.to === clicked)) {
      ...
      selectedSrc = null; validDests = []; combinedDests = [];
      const applyNext = (steps: Move[], idx: number): void => {
        if (idx >= steps.length) { afterMove(); return; }
    ...
    if (selectedSrc !== null && validDests.includes(clicked as number)) {
      applyMove(selectedSrc, clicked as number);
      selectedSrc = null; validDests = []; combinedDests = [];
      afterMove();
      return;
    }

    const alreadyPickedCount = (src: MoveFrom): number =>
      pickedQueue.filter(x => x === src).length + (selectedSrc === src ? 1 : 0);

    const isOwnBar = clicked === 'bar' && (
      humanColor === 'white' ? barW > alreadyPickedCount('bar')
                             : barB > alreadyPickedCount('bar')
    );
    const isOwnPoint = typeof clicked === 'number' && clicked >= 1 && clicked <= 24 && (
      (humanColor === 'white' ? Math.max(board[clicked], 0) : Math.max(-board[clicked], 0))
    ...
    if (isOwnBar || isOwnPoint) {
      ...
      if (totalPicked < movesLeft.length) {
        if (selectedSrc === null) {
          selectedSrc = clicked as MoveFrom;
        } else {
          pickedQueue.push(clicked as MoveFrom);
        }
      }
    } else {
      ...
      if (isAnyOwn && alreadyPickedCount(clicked as MoveFrom) > 0) {
        const queueIdx = pickedQueue.lastIndexOf(clicked as MoveFrom);
        if (queueIdx >= 0) {
    ...
    if (selectedSrc !== null) {
      ...
      const extras: string[] = [];
      if (combinedDests.length > 0) extras.push(`+${combinedDests.length} combined`);
      if (pickedQueue.length > 0) extras.push(`${pickedQueue.length} queued`);
  ...
  const drawBoard = (): void => {
    ...
    ctx.strokeStyle = S.boardBorder; ctx.lineWidth = 2;
    ctx.strokeRect(MARGIN, MARGIN, W - MARGIN * 2, H - MARGIN * 2);

    ctx.fillStyle = S.barBg;
    ctx.fillRect(BAR_X, MARGIN, BAR_W, H - MARGIN * 2);
    ctx.strokeStyle = S.boardBorder; ctx.lineWidth = 1;
    ...
    ctx.strokeStyle = S.midLine; ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(MARGIN, H / 2); ctx.lineTo(BEAR_X, H / 2);
    ctx.stroke();

    const drawMedallion = (mx: number, my: number): void => {
      ctx.save();
      ctx.translate(mx, my);
  ...
  const drawBearOff = (): void => {
    const bx = BEAR_X + BEAR_W / 2;
    ctx.fillStyle = S.labelGold; ctx.font = S.fontSmallLabel; ctx.textAlign = 'center';
  ...
  const drawMiniChecker = (x: number, y: number, player: Player): void => {
    ctx.beginPath();
  ...
  const drawDie = (x: number, y: number, val: number, isUsed: boolean, rotation = 0): void => {
  ...
  const drawHighlights = (): void => {
    if (selectedSrc === null && pickedQueue.length === 0) return;
    pickedQueue.forEach(src => {
      getValidDestsFrom(src).forEach(d => {
        if (d === 0) drawBearOffHighlight('white');
        else if (d === 25) drawBearOffHighlight('black');
        else drawQueuedDestHighlight(d);
      });
    });
    if (selectedSrc !== null) drawSourceHighlight(selectedSrc);
    pickedQueue.forEach(src => drawQueuedHighlight(src));
    validDests.forEach(d => {
      if (d === 0) drawBearOffHighlight('white');
      else if (d === 25) drawBearOffHighlight('black');
      else drawDestHighlight(d);
    });
    combinedDests.forEach(c => {
      if (c.to === 0) drawBearOffHighlight('white');
      else if (c.to === 25) drawBearOffHighlight('black');
      else drawCombinedDestHighlight(c.to);
    });
  };

  const drawSourceHighlight = (src: MoveFrom): void => {
    ctx.strokeStyle = S.hlSrc;
    ctx.lineWidth = 3;
    ctx.shadowBlur = 12; ctx.shadowColor = S.hlSrc;
    if (src === 'bar') {
  ...
  const drawQueuedHighlight = (src: MoveFrom): void => {
    ctx.strokeStyle = S.hlQueued;
    ctx.lineWidth = 3;
    ctx.shadowBlur = 10; ctx.shadowColor = S.hlQueuedShadow;
    if (src === 'bar') {
  ...
  const drawDestHighlight = (dest: number): void => {
    const cx = pointCenterX(dest);
    ctx.fillStyle = S.hlDest;
    ctx.shadowBlur = 10; ctx.shadowColor = S.hlDestShadow;
    ctx.beginPath();
    if (isTop(dest)) {
      ctx.moveTo(cx - PW / 2, BOARD_TOP);
      ctx.lineTo(cx + PW / 2, BOARD_TOP);
  ...
  const drawCombinedDestHighlight = (dest: number): void => {
    const cx = pointCenterX(dest);
    ctx.fillStyle = S.hlCombined;
    ctx.shadowBlur = 10; ctx.shadowColor = S.hlCombinedShadow;
    ctx.beginPath();
    if (isTop(dest)) {
      ctx.moveTo(cx - PW / 2, BOARD_TOP);
      ctx.lineTo(cx + PW / 2, BOARD_TOP);
  ...
  const drawQueuedDestHighlight = (dest: number): void => {
    const cx = pointCenterX(dest);
    ctx.fillStyle = S.hlQueuedDest;
    ctx.shadowBlur = 6; ctx.shadowColor = S.hlQueuedDestShadow;
    ctx.beginPath();
    if (isTop(dest)) {
      ctx.moveTo(cx - PW / 2, BOARD_TOP);
      ctx.lineTo(cx + PW / 2, BOARD_TOP);
  ...
  const drawBearOffHighlight = (color: Player): void => {
  ...
  const drawOpeningDie = (x: number, y: number, val: number, isHuman: boolean): void => {
