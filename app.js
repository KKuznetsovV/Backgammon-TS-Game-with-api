...
const pickGeminiSequence = async (cColor) => {
  ...
  const boardLines = [];
  for (let p = 1; p <= 24; p++) {
    if (board[p] > 0) boardLines.push(`Point ${p}: ${board[p]} white`);
    else if (board[p] < 0) boardLines.push(`Point ${p}: ${Math.abs(board[p])} black`);
  }
...
const evaluateEnhanced = (cColor) => {
  ...
  let run = 0, maxRun = 0;
  for (let p = 1; p <= 24; p++) {
    run = (isB ? board[p] <= -2 : board[p] >= 2) ? run + 1 : 0;
  ...
  for (let p = 1; p <= 24; p++) {
    ...
    for (let d = 1; d <= 6; d++) {
      if (isB) {
        if (p + d <= 24 && board[p + d] > 0) shots++;
        if (barW > 0 && (25 - d) === p) shots++;
      } else {
        if (p - d >= 1 && board[p - d] < 0) shots++;
        if (barB > 0 && d === p) shots++;
      }
    }
  ...
  for (let p = isB ? 1 : 7; p <= (isB ? 18 : 24); p++) {
...
const DICE_POOL = (() => {
  ...
  for (let a = 1; a <= 6; a++)
    for (let b = a; b <= 6; b++)
      pool.push({ d: a === b ? [a,a,a,a] : [a,b], w: a === b ? 1 : 2 });
  return pool;
})();
  ...
  const dfs = (from, steps) => {
    ...
    for (const move of moves) {
      ...
      if (newSteps.length >= 2) {
        if (!combined.some(c => c.to === move.to)) {
...
const hitTest = (mx, my) => {
  ...
  return Array.from({ length: 24 }, (_, i) => i + 1).reduce((found, p) => {
...
const drawBoard = () => {
  ...
  Array.from({ length: 12 }, (_, i) => i + 13).forEach(p =>
    ctx.fillText(p, pointCenterX(p), MARGIN - 6)
  );
  Array.from({ length: 12 }, (_, i) => i + 1).forEach(p =>
    ctx.fillText(p, pointCenterX(p), H - 6)
  );
};
...
const drawBearOff = () => {
  ...
  if (offW > 0) {
    ...
    ctx.fillText(offW, bx, whiteStartY + 10);
  }
  ...
  if (offB > 0) {
    ctx.fillStyle = S.labelLight; ctx.font = S.fontBorneCount; ctx.textAlign = 'center';
    ctx.fillText(offB, bx, blackStartY + 10 + Math.ceil(Math.min(offB, 10) / 2) * 16);
  }
};
